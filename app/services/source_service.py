...
from app.utils.http_fetch import fetch_text
    ...
    async def load(self, source: TutorSource, *, force_refresh: bool = False) -> LoadedSource:
        ...
        if document is not None and document.is_fresh() and not force_refresh:
            if document.fetch_error:
                return LoadedSource(source=source, text="", outline=[], error=document.fetch_error)
            return LoadedSource(source=source, text=document.text, outline=document.outline)

        return await self._refresh(source, document)

    async def _refresh(self, source: TutorSource, document: SourceDocument | None) -> LoadedSource:
        ...
        try:
            ...
            return LoadedSource(
                ...
                error=None if document and document.text else exc.message,
            )
